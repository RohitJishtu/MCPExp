...
try:
    ...
    mcp_client = MCPClient({"url": "http://127.0.0.1:7861/gradio_api/mcp/sse"})
    # Retrieve the tool list registered on the MCP server (e.g., sentiment_analysis)
    tools = mcp_client.get_tools()
    # Use the OpenAI model exposed by the MCP server (e.g., gpt-4o)
...
"""
    ...
    demo = gr.ChatInterface(
        fn=smart_agent, # Function to handle messages
        # type="messages", # Accept message-based input
        examples=["Analyze the sentiment of the following text: 'This is awesome'"], # Pre-filled example for testing
        title="Agent with MCP Tools",  # UI Title
        description="Smart agent: quick replies for simple asks, full reasoning for complex ones.", # UI Description
    )
